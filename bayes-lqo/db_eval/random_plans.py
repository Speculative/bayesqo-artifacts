...
def generate_random_plan(query_join_graph: nx.Graph, workload: WorkloadSpecDefinition, plan_type: PlanType) -> JoinTree:
    # Sample edges until the graph is re-connected
    partial_joins = AliasSymbolTable(workload.all_tables, expand_counts(workload.all_tables, workload.query_tables))
    ...
    while len(partial_joins.tree_to_symbols) > 1:
        ...
        if left_subtree is None or right_subtree is None:
            raise ValueError(f"Couldn't find {left_table} {left_alias} or {right_table} {right_alias}")

        # Sometimes edges will cause cycles
        if left_subtree is right_subtree:
            continue

        new_tree = JoinTreeBranch(left_subtree, right_subtree, choice(list(JoinOperator)))
...
def build_query_join_graph(aliased_join_graph: nx.Graph, workload: WorkloadSpecDefinition) -> nx.Graph:
    query_table_aliases = set(
        (table, alias_num) for (table, num_aliases) in workload.query_tables for alias_num in range(1, num_aliases + 1)
    )
...
def try_random_plans(
    ...
    initial_timeout: Union[Literal["pg"], Literal["bao"], Literal["constant"]] = "constant",
):
    ...
    if run_best_seen.runtime_secs:
        ...
        if past_initial_timeout:
            ...
            l.info(f"No successful plans in previous run, doubling initial best to {pretty_time(best_seen)}")
    else:
        match initial_timeout:
            case "pg":
                postgres_avg = postgres_time(query)
                if postgres_avg:
                    l.info(f"Using average postgres time of {postgres_avg} as initial best")
    ...
    run_time_count = (
        ...
        .where((RandomPlan.plan_type == plan_type) & (RandomPlan.query_name == query) & (RandomPlan.run_id == run_id))
        .scalar()
    ) or 0

    if max_plans is not None:
        l.info(f"Starting run {run_id} for {query}, {run_plan_count} plans already evaluated")
    ...
    while True:
        ...
        plan = generate_random_plan(query_join_graph, workload, plan_type)
        ...
        if existing > 0:
            l.info("Skipping existing plan")
            continuous_skips += 1
            if continuous_skips > 1000:
                l.info("Too many continuous skips, stopping")
        ...
        num_plans = (
            RandomPlan.select()
            .where(
                (RandomPlan.plan_type == plan_type) & (RandomPlan.query_name == query) & (RandomPlan.run_id == run_id)
            )
            .count()
        )
        ...
        match result:
            ...
            case FailedQuery(_, elapsed_secs, error):
                l.info(f"{run_plan_count}: Failed query after {pretty_time(elapsed_secs)}: {error}")
                if isinstance(error, str) and "no space" in error:
                    RandomPlan.create(
                        plan_type=plan_type,
                        query_name=query,
                        run_id=run_id,
...
def all_random_series(query: str, max_samples: Optional[int] = None) -> list[list[tuple[float, float]]]:
    ...
    """
    highest_run_number = RandomPlan.select(fn.MAX(RandomPlan.run_id)).where(RandomPlan.query_name == query).scalar()
    runs = []
...
def best_random_series(query: str, max_samples: Optional[int] = None) -> list[tuple[float, float]]:
    """
...
def fill_ceb(
    ...
    print(f"Optimizing {len(work)} queries")
    shuffle(work)
    for query in work:
        optimize_until(
            workload_set="CEB_3K",
    ...
    print(f"Optimized {len(work)} queries")


@app.command()
def clear_ceb():
    count = RandomPlan.select().where(RandomPlan.workload_set == "CEB_3K").group_by(RandomPlan.query_name).count()
    print(f"Before: {count} queries")
    input("Are you sure?")
    top, _ = top_bot_improvement("CEB_3K", 100)
    RandomPlan.delete().where((RandomPlan.query_name << top) & (RandomPlan.workload_set == "CEB_3K")).execute()
    count = RandomPlan.select().where(RandomPlan.workload_set == "CEB_3K").group_by(RandomPlan.query_name).count()
...
def plot(
    ...
    for run_id in range(1, max_run.run_id + 1):
        runs = (
            RandomPlan.select(
                ...
                RandomPlan.result,
                fn.Sum(RandomPlan.runtime_secs).over(order_by=[RandomPlan.plan_id]).alias("cumulative_time"),
            )
            .where(
                (RandomPlan.plan_type == plan_type) & (RandomPlan.query_name == query) & (RandomPlan.run_id == run_id)
            )
    ...
    plt.axhline(y=postgres_time(query), color="orange", linestyle="--", label="Postgres")
...
def plot_all(plan_type: PlanType = PlanType.NonCrossUniformOp):
    ...
    for query in tqdm(JOB_QUERIES_SORTED):
        if (
            RandomPlan.select().where((RandomPlan.plan_type == plan_type) & (RandomPlan.query_name == query)).count()
            > 0
        ):
    ...
    for query in JOB_QUERIES_SORTED:
        ...
        if best_run is not None:
            ...
            for plan in plans_in_run:
                print(f"{query};{plan.result};{plan.runtime_secs};{','.join(str(c) for c in plan.encoded_plan)}")
...
def ceb_top_vs_bao():
    ...
    by_improvement = sorted(
        [(query, bao_optimal_time(query, "CEB_3K") / postgres_time(query)) for query in workload_set.queries],
        key=lambda x: x[1],
    )
    ...
    for category, queries in [("Top 50", top_50), ("Bottom 50", bot_50)]:
        for query, _ in queries:
            random_run = (
                ...
                .where((RandomPlan.query_name == query) & (RandomPlan.plan_type == PlanType.NonCrossUniformOp))
                .order_by(RandomPlan.plan_id)
            )
            best_seen = float("inf")
            had_success = False
